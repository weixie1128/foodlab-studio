...
const defaultChartSettings = {
  ...
  wfLayerDx:16, wfLayerDy:20, wfFill:true, wfFillOpacity:.16, wfZLabels:true, wfLineEndLabels:false, wfZLabelSize:11, wfZAxisTitle:'Time', zTitleSize:12,
  background:'#ffffff'
};
...
function init(){
  ...
  renderDataPreview();
  showView('plan');
  if('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v='+(window.FOODLAB_BUILD||'0.78.0')).catch(()=>{});
}
...
function parseStructuredExperimentMatrix(matrix){
  ...
  // v0.78.0 单层宽表守卫：第二行若是实验数值（而非每组连续的平行编号/标签），
...
function renderChart(){
  ...
  if(state.chart.type==='waterfall'){const WfM=wfMargins(gs.length);const WfPW=W-WfM.l-WfM.r,WfPH=H-WfM.t-WfM.b;svg+=renderWaterfall(W,H,WfM,WfPW,WfPH,xvals,gs,colors,bounds);}
  else{svg+=state.chart.breakAxis?renderBrokenPlot(W,H,M,plotW,plotH,xvals,gs,colors):renderNormalPlot(W,H,M,plotW,plotH,xvals,gs,colors,bounds);}
...
/* ===== v0.78.0 光谱瀑布图（3D 堆叠折线，Origin 风格）=====
 * 数据：第一列=波长/X（连续），其余每列=一个 Z 层（时间点）。
 * 渲染：第 k 层整体沿 (k*dx, k*dy) 斜向偏移，半透明填充到本层基线；
 *       Y 轴只画最前层左侧，X 轴只画最前层底部，右侧 Z 轴斜线标层名。
 */
function wfMargins(n){
  const s=state.chart.settings, dx=clamp(s.wfLayerDx||0,0,90), dy=clamp(s.wfLayerDy||0,0,90);
  const last=Math.max(0,n-1);
  return {l:108, r:108+last*dx, t:88+last*dy, b:108};
}
function renderWaterfall(W,H,M,plotW,plotH,xvals,gs,colors,b){
  const s=state.chart.settings, model=getChartModel(), xCfg=experimentXAxisConfig(xvals,M,plotW),
    y=yValueMap(b,plotH,M.t), axisY=M.t+plotH,
    dx=clamp(s.wfLayerDx||0,0,90), dy=clamp(s.wfLayerDy||0,0,90), n=gs.length;
  let out='';
  for(let k=n-1;k>=0;k--){
    const g=gs[k], pts=model.byGroup.get(String(g))||[], c=colors[k%colors.length];
    const coords=pts.map(d=>{
      const xx=xCfg?xCfg.pos(Number(convertXValue(d.x))):M.l+(model.xIndex.get(String(d.x))??0)*(plotW/xvals.length);
      return [xx+k*dx, y(d.mean)-k*dy];
    }).filter(p=>Number.isFinite(p[1]));
    if(coords.length<2)continue;
    if(s.wfFill){
      const baseY=axisY-k*dy;
      out+=`<path data-object="series" data-series="${k}" d="M${coords[0][0].toFixed(1)},${baseY.toFixed(1)} L${coords.map(p=>p[0].toFixed(1)+','+p[1].toFixed(1)).join(' L')} L${coords[coords.length-1][0].toFixed(1)},${baseY.toFixed(1)} Z" fill="${c}" fill-opacity="${s.wfFillOpacity}" stroke="none"/>`;
    }
    ...
    if(s.wfLineEndLabels){const lp=coords[coords.length-1];out+=`<text data-object="z-label" x="${lp[0]+6}" y="${lp[1]+4}" font-size="${s.wfZLabelSize}" fill="${c}">${esc(g)}</text>`;}
  }
  ...
  if(s.showYTicks)yTicks.forEach(v=>{const yy=y(v);out+=`<line x1="${M.l-s.tickLength}" x2="${M.l}" y1="${yy}" y2="${yy}"/>`});
  out+='</g>';
  ...
  const tickObjs=experimentXAxisTickObjects(xvals,M,plotW);
  if(s.showXTicks)tickObjs.forEach(t=>{out+=`<line x1="${t.x}" x2="${t.x}" y1="${axisY}" y2="${axisY+s.tickLength}"/>`});
  out+='</g>';
  const xlabs=tickObjs.map(t=>t.label), rot=automaticXTickRotation(xlabs);
  tickObjs.forEach(t=>{const yy=axisY+s.tickLength+18;out+=`<text data-object="axis-x" class="chart-object" x="${t.x}" y="${yy}" text-anchor="${rot<0?'end':'middle'}" font-size="${s.xTickSize}" fill="${s.xTickColor}" transform="rotate(${rot} ${t.x} ${yy})">${esc(t.label)}</text>`});
  if(n>1&&s.wfZLabels){
    const zx2=M.l+plotW+(n-1)*dx, zy2=axisY-(n-1)*dy;
    out+=`<g data-object="axis-z" class="chart-object" stroke="${s.axisColor}" stroke-width="${s.axisWidth}" fill="none"><path d="M${M.l+plotW},${axisY} L${zx2},${zy2}"/></g>`;
    for(let k=0;k<n;k++){
      const zx=M.l+plotW+k*dx, zy=axisY-k*dy;
      out+=`<line x1="${zx}" x2="${zx+7}" y1="${zy}" y2="${zy}" stroke="${s.axisColor}" stroke-width="${s.axisWidth}"/>`;
      out+=`<text data-object="axis-z" x="${zx+11}" y="${zy+4}" font-size="${s.wfZLabelSize}" fill="${s.axisColor}">${esc(gs[k])}</text>`;
    }
    if(s.wfZAxisTitle){const mx=(M.l+plotW+zx2)/2,my=(axisY+zy2)/2;out+=`<text data-object="axis-z" x="${mx+16}" y="${my}" font-size="${s.zTitleSize||s.yTickSize}" font-style="italic" fill="${s.axisColor}">${esc(s.wfZAxisTitle)}</text>`;}
  }
