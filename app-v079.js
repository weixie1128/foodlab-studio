...
const defaultChartSettings = {
  ...
  barGap:3, categoryWidth:.72, barOpacity:.96, barBorderWidth:.55, barLineMode:false,barLineWidth:1.8,barLineMarker:true,
  ...
  wfLayerDx:34, wfLayerDy:46, wfFill:true, wfFillOpacity:.08, wfZLabels:true, wfLineEndLabels:false, wfZLabelSize:11, wfZAxisTitle:'Time', zTitleSize:12,
  background:'#ffffff'
};
...
function init(){
  ...
  renderDataPreview();
  showView('plan');
  if('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v='+(window.FOODLAB_BUILD||'0.79.0')).catch(()=>{});
}
...
const PLAN_CHART_META={
  ...
  waterfall:{group:'趋势与组间差异',icon:'≋',purpose:'多层光谱/曲线沿纵深堆叠，展示随时间或条件的演化',analysis:'每层一条曲线，层间自动错开，无需重复',advice:'紫外/荧光光谱随时间变化的论文常用图：第一列填波长/横轴，其余每列一个时间点',schema:'光谱宽表（第一列波长）'},
...
function parseStructuredExperimentMatrix(matrix){
  ...
  // v0.79.0 单层宽表守卫：第二行若是实验数值（而非每组连续的平行编号/标签），
...
function prepareChartData(){
  ...
  if(d.designType==='one'){
    ...
    xLevels.forEach(x=>{
      ...
      const letters=((state.chart.type==='curve'||state.chart.type==='waterfall'||state.analysis?.continuous||xLevels.length>250))?{}:lettersForComparisons(comps,state.analysis?.mse,state.analysis?.dfError);
...
function gallerySpecificLayerIds(type=state.gallery.type){
  const map={
    ...
    scatter:[['regression','拟合线与相关系数'],['method-note','方法说明']],bubble:[['regression','拟合线与相关系数'],['bubble-size','气泡大小'],['method-note','方法说明']],stacked:[['stack-mode','堆叠方式'],['stacked-error','误差棒与数值标注']],pie:[['pie-label','比例标签']],
...
function defaultSeriesStyle(index){
  const s=state.chart.settings;
  return {lineWidth:s.lineWidth,markerSize:s.markerSize,markerShape:SERIES_MARKERS[index%SERIES_MARKERS.length],markerFill:s.markerFill,barLine:true};
}
...
/* ===== v0.79.0 光谱瀑布图（3D 堆叠折线，Origin 风格）=====
 * 第一列=连续 X（波长/时间），其余每列=一个 Z 层。
 * 每层片沿 (dx,-dy) 错开，曲线下淡填充到本层基线，并画片层基线形成"地面"；
 * 只画最前层 X/Y 轴，右侧 Z 轴斜线标层名；不画通用图例（层名已在 Z 轴）。
 */
function wfLayout(n,W,H){
  const s=state.chart.settings;
  let dx=clamp(Number(s.wfLayerDx)||0,0,160),dy=clamp(Number(s.wfLayerDy)||0,0,160);
  if(n>1){dx=Math.min(dx,Math.max(0,(W-300))*0.62/(n-1));dy=Math.min(dy,Math.max(0,(H-210))*0.62/(n-1));}
  return {l:96,r:120+(n-1)*dx,t:66+(n-1)*dy,b:94,dx,dy};
}
function wfXScale(xvals,M,plotW){
  const s=state.chart.settings, nums=xvals.map(Number), numeric=nums.length>1&&nums.every(Number.isFinite);
  if(numeric){
    let lo=Math.min(...nums),hi=Math.max(...nums);const span=(hi-lo)||1;const pad=span*0.02;lo-=pad;hi+=pad;
    const pos=v=>M.l+(Number(v)-lo)/(hi-lo)*plotW;
    const seg=clamp(Math.round(Number(s.xAxisSegments)||8),2,20);const step=niceAxisStep(hi-lo,seg);
    const ticks=[];for(let v=Math.ceil(lo/step)*step;v<=hi+step*0.5;v+=step)ticks.push({x:pos(v),label:formatAxisNumber(v,s.xTickDecimals,step,s.xTickRound)});
    return {pos,ticks};
  }
  const step=plotW/xvals.length, idx={};xvals.forEach((v,i)=>idx[String(v)]=i);
  const pos=v=>M.l+((idx[String(v)]??0)+.5)*step;
  return {pos,ticks:xvals.map((v,i)=>({x:M.l+(i+.5)*step,label:formatXTick(v)}))};
}
function renderWaterfall(W,H,M,plotW,plotH,xvals,gs,colors,b){
  const s=state.chart.settings, model=getChartModel(), xs=wfXScale(xvals,M,plotW),
    y=yValueMap(b,plotH,M.t), axisY=M.t+plotH,
    dx=M.dx, dy=M.dy, n=gs.length;
  let out='';
  for(let k=n-1;k>=0;k--){
    const g=gs[k],pts=model.byGroup.get(String(g))||[],c=colors[k%colors.length];
    const coords=pts.map(d=>[xs.pos(d.x),y(d.mean)-k*dy]).filter(p=>Number.isFinite(p[0])&&Number.isFinite(p[1]));
    if(coords.length<2)continue;
    const baseY=axisY-k*dy,x0=coords[0][0],x1=coords[coords.length-1][0];
    if(s.wfFill)out+=`<path data-object="series" data-series="${k}" d="M${x0.toFixed(1)},${baseY.toFixed(1)} L${coords.map(p=>p[0].toFixed(1)+','+p[1].toFixed(1)).join(' L')} L${x1.toFixed(1)},${baseY.toFixed(1)} Z" fill="${c}" fill-opacity="${s.wfFillOpacity}" stroke="none"/>`;
    out+=`<line data-object="axis-z" x1="${x0.toFixed(1)}" y1="${baseY.toFixed(1)}" x2="${x1.toFixed(1)}" y2="${baseY.toFixed(1)}" stroke="${s.axisColor}" stroke-opacity=".3" stroke-width="1"/>`;
    const lw=(getSeriesStyle(k).lineWidth)||s.lineWidth;
    out+=`<path data-object="series" data-series="${k}" class="chart-object" d="${coords.map((p,i)=>(i?'L':'M')+p[0].toFixed(1)+','+p[1].toFixed(1)).join(' ')}" fill="none" stroke="${c}" stroke-width="${lw}" stroke-linejoin="round" stroke-linecap="round"/>`;
    if(s.wfLineEndLabels)out+=`<text data-object="z-label" x="${(x1+6).toFixed(1)}" y="${(coords[coords.length-1][1]+4).toFixed(1)}" font-size="${s.wfZLabelSize}" fill="${c}">${esc(g)}</text>`;
  }
  const yTicks=chartYTicks(b),yStep=yTicks.length>1?(yTicks[1]-yTicks[0]):1;
  ...
  if(s.showYTicks)yTicks.forEach(v=>{const yy=y(v);out+=`<line x1="${M.l-s.tickLength}" x2="${M.l}" y1="${yy.toFixed(1)}" y2="${yy.toFixed(1)}"/>`});
  out+='</g>';
  yTicks.forEach(v=>{const yy=y(v);out+=`<text data-object="axis-y" class="chart-object" x="${M.l-s.tickLength-6}" y="${(yy+4).toFixed(1)}" text-anchor="end" font-size="${s.yTickSize}" fill="${s.yTickColor}">${formatAxisNumber(v,s.yTickDecimals,yStep,false)}</text>`});
  ...
  if(s.showXTicks)xs.ticks.forEach(t=>{out+=`<line x1="${t.x.toFixed(1)}" x2="${t.x.toFixed(1)}" y1="${axisY}" y2="${axisY+s.tickLength}"/>`});
  out+='</g>';
  const rot=automaticXTickRotation(xs.ticks.map(t=>t.label));
  xs.ticks.forEach(t=>{const yy=axisY+s.tickLength+18;out+=`<text data-object="axis-x" class="chart-object" x="${t.x.toFixed(1)}" y="${yy}" text-anchor="${rot<0?'end':'middle'}" font-size="${s.xTickSize}" fill="${s.xTickColor}" transform="rotate(${rot} ${t.x.toFixed(1)} ${yy})">${esc(t.label)}</text>`});
  if(n>1&&s.wfZLabels){
    const zx1=M.l+plotW,zy1=axisY,zx2=zx1+(n-1)*dx,zy2=axisY-(n-1)*dy;
    out+=`<g data-object="axis-z" class="chart-object" stroke="${s.axisColor}" stroke-width="${s.axisWidth}" fill="none"><path d="M${zx1},${zy1} L${zx2.toFixed(1)},${zy2.toFixed(1)}"/></g>`;
    for(let k=0;k<n;k++){const zx=zx1+k*dx,zy=axisY-k*dy;
      out+=`<line x1="${zx.toFixed(1)}" x2="${(zx+7).toFixed(1)}" y1="${zy.toFixed(1)}" y2="${zy.toFixed(1)}" stroke="${s.axisColor}" stroke-width="${s.axisWidth}"/>`;
      out+=`<text data-object="axis-z" x="${(zx+11).toFixed(1)}" y="${(zy+4).toFixed(1)}" font-size="${s.wfZLabelSize}" fill="${s.axisColor}">${esc(gs[k])}</text>`;}
    if(s.wfZAxisTitle)out+=`<text data-object="axis-z" x="${(zx2+16).toFixed(1)}" y="${(zy2-8).toFixed(1)}" font-size="${s.zTitleSize||s.yTickSize}" font-style="italic" fill="${s.axisColor}">${esc(s.wfZAxisTitle)}</text>`;
  }
  out+=renderFrame(M,plotW,plotH,false);
  if(s.xTitleVisible&&s.xTitle)out+=`<text data-object="axis-x" data-drag="xTitle" class="chart-object draggable" x="${(M.l+plotW/2).toFixed(1)}" y="${(axisY+62).toFixed(1)}" text-anchor="middle" font-size="${s.xTitleSize}" font-weight="${s.xTitleWeight||s.globalFontWeight||400}" fill="${s.xTitleColor}">${esc(convertedXTitle())}</text>`;
  if(s.yTitleVisible&&s.yTitle)out+=`<text data-object="axis-y" data-drag="yTitle" class="chart-object draggable" transform="translate(${M.l-62} ${(M.t+plotH/2).toFixed(1)}) rotate(-90)" text-anchor="middle" font-size="${s.yTitleSize}" font-weight="${s.yTitleWeight||s.globalFontWeight||400}" fill="${s.yTitleColor}">${esc(s.yTitle)}</text>`;
  return out;
}
...
function renderProperties(){
  ...
  if(id==='title'){name='图题';html=fieldGroup([
    ...
    ...(state.chart.type==='bar'?[checkField('barLineMode','柱线组合（柱顶叠加均值趋势线）'),checkField(`series:${idx}:barLine`,'本系列趋势折线'),rangeField('barLineWidth','趋势线粗细',.8,5,.1),checkField('barLineMarker','趋势线数据点')]:[]), ...(state.chart.type==='waterfall'?[checkField('wfFill','每层填充到基线'),rangeField('wfFillOpacity','填充透明度',0,.8,.02),rangeField('wfLayerDx','层间水平错开（向右）',0,160,1),rangeField('wfLayerDy','层间垂直错开（向上）',0,160,1),checkField('wfZLabels','右侧 Z 轴与层标签'),checkField('wfLineEndLabels','线尾标注层名'),textField('wfZAxisTitle','Z 轴标题'),rangeField('wfZLabelSize','Z 轴标签字号',8,20,1)]:[])
...
const GALLERY_SCHEMAS={
  ...
  composition:{name:'组成数据长表',columns:['Category','Component','Value','Error'],description:'Category 为横坐标类别，Component 为类别内部组成；Error 为误差（SD/SE），用于堆叠图的误差棒，可留空；饼图可将 Category 全部填写为 Overall。'},
...
function downloadGalleryXlsx(){
  ...
  if(def.id==='radar'){
    ...
    guide=XLSX.utils.aoa_to_sheet([
      ['FoodLab Studio Chart Template'],['Chart type',workflowChartEnglishLabel(def.id)],['Template structure',gallerySchemaEnglishName(def.schema)],['Entry rule','Enter raw observations. FoodLab also accepts common wide-table layouts when the first column identifies the group, category, sample, or X variable.'],['Error bars','Stacked bars read the Error column (long table) or a wide-table Error total column.'],
...
function normalizeGalleryRows(rows,schema){
  ...
  if(schema==='composition'){
    ...
    rows.forEach((r,i)=>{
      ...
      const errorKey=keys.find(k=>['error','sd','se','std','stddev','误差','标准差','标准误'].includes(textKey(k).toLowerCase())),rowError=errorKey?numOrNull(r[errorKey]):null;
      keys.forEach(k=>{if(k===cKey||k===errorKey)return;const kk=textKey(k);if(!kk)return;if(['total','合计','总量','总和','总计','sum'].includes(kk.toLowerCase()))return;const n=numOrNull(r[k]);if(n!=null)out.push({Category:cat,Component:kk,Value:n,totalError:rowError})});
    });return out;
  }
  if(schema==='xy'){
    const out=[];
...
function galleryStacked(W,H){
  ...
  comps.forEach((comp,j)=>{const st=getGallerySeriesStyle(j);let body='';cats.forEach((cat,i)=>{const previous=comps.slice(0,j).reduce((sum,c)=>{const r=state.gallery.rows.find(x=>x.Category===cat&&x.Component===c);const v=r?r.Value:0;return sum+(s.normalize?(totals[cat]?v/totals[cat]*100:0):v)},0),row=state.gallery.rows.find(r=>r.Category===cat&&r.Component===comp),value=row?row.Value:0,v=s.normalize?(totals[cat]?value/totals[cat]*100:0):value,y1=yMap(previous+v),y0=yMap(previous),w=Math.min(78,xStep*.68),x=p.l+(i+.5)*xStep-w/2;body+=`<rect x="${x}" y="${y1}" width="${w}" height="${Math.max(0,y0-y1)}" fill="${st.color}" fill-opacity="${st.opacity}" stroke="${st.color}" stroke-width="${st.lineWidth}"/>`;
    if(s.stackedShowValue&&(y0-y1)>=9){const lum=hexLuminance(st.color),val=s.normalize?v:value;body+=`<text x="${x+w/2}" y="${(y0+y1)/2+Number(s.stackedValueSize||12)/3.2}" text-anchor="middle" font-size="${s.stackedValueSize||12}" font-weight="600" fill="${lum>.55?'#111111':'#ffffff'}">${formatNumber(val,s.stackedValueDecimals??1)}</text>`}
  ...
  if(s.stackedShowError){let eb='';cats.forEach((cat,i)=>{const x=p.l+(i+.5)*xStep,catRows=state.gallery.rows.filter(r=>r.Category===cat);
    ...
    if(totalErr>0){const yT=yMap((s.normalize?100:totals[cat])+totalErr),yB=yMap(s.normalize?100:totals[cat]),ew=Number(s.stackedErrorWidth)||1,cap=Number(s.stackedErrorCapWidth)||ew*2.4,c=s.stackedErrorColor||'#263238';eb+=`<line data-gobject="error-bars" x1="${x}" y1="${yB}" x2="${x}" y2="${Math.min(yT,yB)}" stroke="${c}" stroke-width="${ew}"/><line data-gobject="error-bars" x1="${x-cap/2}" y1="${Math.min(yT,yB)}" x2="${x+cap/2}" y2="${Math.min(yT,yB)}" stroke="${c}" stroke-width="${ew}"/>`}
  });out+=`<g data-gobject="error-bars" class="chart-object">${eb}</g>`}
  return out;
}
function hexLuminance(hex){const h=hexRgb(hex).map(v=>v/255);return .2126*h[0]+.7152*h[1]+.0722*h[2]}
