...
const defaultChartSettings = {
  ...
  wfLayerDx:64, wfLayerDy:80, wfFill:true, wfFillOpacity:.55, wfZLabels:true, wfLineEndLabels:false, wfZLabelSize:11, wfZAxisTitle:'Time', zTitleSize:12,
  background:'#ffffff'
};
...
function init(){
  ...
  renderDataPreview();
  showView('plan');
  if('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v='+(window.FOODLAB_BUILD||'0.82.0')).catch(()=>{});
}
...
function parseStructuredExperimentMatrix(matrix){
  ...
  // v0.82.0 单层宽表守卫：第二行若是实验数值（而非每组连续的平行编号/标签），
...
/* ===== v0.82.0 光谱瀑布图（真 3D 坐标盒，Matlab/Origin 风格）=====
