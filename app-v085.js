...
function init(){
  ...
  renderDataPreview();
  showView('plan');
  if('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v='+(window.FOODLAB_BUILD||'0.85.0')).catch(()=>{});
}
...
function parseStructuredExperimentMatrix(matrix){
  ...
  // v0.85.0 单层宽表守卫：第二行若是实验数值（而非每组连续的平行编号/标签），
...
function syncChartText(){
  if(state.chart.type==='waterfall')state.chart.xFactor='A';
...
/* ===== v0.85.0 光谱瀑布图（真 3D 坐标盒，Matlab/Origin 风格）=====
...
function renderWaterfall(W,H,M,plotW,plotH,xvals,gs,colors,b){
  ...
  // 9) 轴标题
  if(s.xTitleVisible&&s.xTitle){const p=P(.5,0,0);out+=`<text data-object="axis-x" data-drag="xTitle" class="chart-object draggable" x="${p[0].toFixed(1)}" y="${(p[1]+64).toFixed(1)}" text-anchor="middle" font-size="${s.xTitleSize}" font-weight="${s.xTitleWeight||s.globalFontWeight||400}" fill="${s.xTitleColor}">${esc(convertedXTitle())}</text>`;}
  if(s.yTitleVisible&&s.yTitle){const p=P(sideNx,.5,0);const rotY=dir>0?-90:90;
    out+=`<text data-object="axis-y" data-drag="yTitle" class="chart-object draggable" transform="translate(${p[0].toFixed(1)} ${p[1].toFixed(1)}) rotate(${rotY})" text-anchor="middle" font-size="${s.yTitleSize}" font-weight="${s.yTitleWeight||s.globalFontWeight||400}" fill="${s.yTitleColor}">${esc(s.yTitle)}</text>`;}
  // 10) 图例
  if(s.wfLegend&&K>=1){
