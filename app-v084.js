...
const defaultChartSettings = {
  ...
  wfLayerDx:64, wfLayerDy:80, wfFill:true, wfFillOpacity:.55, wfZLabels:true, wfLineEndLabels:false, wfZLabelSize:11, wfZAxisTitle:'', zTitleSize:12, wfWallColor:'#f7f6f1', wfGridColor:'#d9d7ce', wfGridWidth:.8, wfLegend:true, wfLegendSize:11,
  background:'#ffffff'
};
...
function init(){
  ...
  renderDataPreview();
  showView('plan');
  if('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v='+(window.FOODLAB_BUILD||'0.84.0')).catch(()=>{});
}
...
function parseStructuredExperimentMatrix(matrix){
  ...
  // v0.84.0 单层宽表守卫：第二行若是实验数值（而非每组连续的平行编号/标签），
...
function renderLayers(){
  const gs=chartGroups();const layers=[['section','基础对象'],['title','图题','base'],['subtitle','副标题','base'],['typography','中英文字体','base'],['canvas','画布与清晰度','base'],['legend','图例内容','base'],['legend-frame','图例边框','base'],['axis-y','Y 轴与纵标题','base'],['axis-x','X 轴与横标题','base'],['frame','图片边框','base'],['background','背景','base'],...(state.chart.type==='waterfall'?[['waterfall3d','3D 瀑布图','base']]:[]),['section','数据对象']];
...
/* ===== v0.84.0 光谱瀑布图（真 3D 坐标盒，Matlab/Origin 风格）=====
...
function wfLayout(n,W,H){
  ...
  const l=98,b=94,r0=108,t0=62;
  dx=Math.min(dx,Math.max(0,(W-l-r0-200))/Math.max(1,K));
  dy=Math.min(dy,Math.max(0,(H-t0-b-170))/Math.max(1,K));
  return {l,t:t0+K*dy,b,r:r0+K*dx,dx,dy};
}
function wfXScale(xvals){
  ...
  if(numeric){
    ...
    const ticks=[];for(let v=Math.ceil(lo/step)*step;v<=hi+step*0.5;v+=step)ticks.push({n:clamp(norm(v),0,1),label:formatAxisNumber(v,s.xTickDecimals,step,s.xTickRound)});
    return {norm,ticks};
  }
  const idx={};xvals.forEach((v,i)=>idx[String(v)]=i);
  return {norm:v=>((idx[String(v)]??0)+0.5)/xvals.length,ticks:xvals.map((v,i)=>({n:(i+0.5)/xvals.length,label:formatXTick(v)}))};
}
...
function renderWaterfall(W,H,M,plotW,plotH,xvals,gs,colors,b){
  if(state.chartData.every(d=>d.mean>=0))b=Object.assign({},b,{min:0});
  {const step=niceAxisStep(b.max-b.min,6);b=Object.assign({},b,{max:Math.ceil(b.max/step)*step});}
  const yStepN=niceAxisStep(b.max-b.min,6);const yTicksNice=[];for(let v=0;v<=b.max+yStepN*.5;v+=yStepN)yTicksNice.push(v);
  const s=state.chart.settings,model=getChartModel(),xs=wfXScale(xvals),
    K=gs.length-1,axisY=M.t+plotH,dx=M.dx,dy=M.dy,
    yN=v=>(Number(v)-b.min)/((b.max-b.min)||1),
    P=(nx,ny,k)=>[M.l+nx*plotW+k*dx,axisY-ny*plotH-k*dy];
  let out='';
  // 1) 后墙（z=K，淡黄）
  out+=wfPoly([P(0,0,K),P(1,0,K),P(1,1,K),P(0,1,K)],'fill="'+s.wfWallColor+'" stroke="none"');
  xs.ticks.forEach(t=>out+=wfLine(P(t.n,0,K),P(t.n,1,K),s.wfGridColor,s.wfGridWidth));
  yTicksNice.forEach(v=>{const ny=yN(v);if(ny>=0&&ny<=1)out+=wfLine(P(0,ny,K),P(1,ny,K),s.wfGridColor,s.wfGridWidth);});
  // 2) 左墙（x=0，淡青）
  out+=wfPoly([P(0,0,0),P(0,0,K),P(0,1,K),P(0,1,0)],'fill="'+s.wfWallColor+'" stroke="none"');
  yTicksNice.forEach(v=>{const ny=yN(v);if(ny>=0&&ny<=1)out+=wfLine(P(0,ny,0),P(0,ny,K),s.wfGridColor,s.wfGridWidth);});
  for(let k=0;k<=K;k++)out+=wfLine(P(0,0,k),P(0,1,k),s.wfGridColor,s.wfGridWidth);
  // 3) 底面（y=0，灰白）
  out+=wfPoly([P(0,0,0),P(1,0,0),P(1,0,K),P(0,0,K)],'fill="'+s.wfWallColor+'" stroke="none"');
  xs.ticks.forEach(t=>out+=wfLine(P(t.n,0,0),P(t.n,0,K),s.wfGridColor,s.wfGridWidth));
  for(let k=0;k<=K;k++)out+=wfLine(P(0,0,k),P(1,0,k),s.wfGridColor,s.wfGridWidth);
  // 4) 各层片（后→前，实色遮挡）
  for(let k=K;k>=0;k--){
  ...
  out+=`<g data-object="axis-box" class="chart-object" stroke="${ac}" stroke-width="${aw}" fill="none" stroke-linecap="square">`
    +`<path d="M${P(0,0,0)[0].toFixed(1)},${P(0,0,0)[1].toFixed(1)} L${P(1,0,0)[0].toFixed(1)},${P(1,0,0)[1].toFixed(1)} L${P(1,0,K)[0].toFixed(1)},${P(1,0,K)[1].toFixed(1)}"/>`
    +`<path d="M${P(0,0,0)[0].toFixed(1)},${P(0,0,0)[1].toFixed(1)} L${P(0,1,0)[0].toFixed(1)},${P(0,1,0)[1].toFixed(1)}"/>`
    +`</g>`;
  out+=`<g stroke="#c9c9c9" stroke-width=".6" fill="none">`
    +`<path d="M${P(1,0,K)[0].toFixed(1)},${P(1,0,K)[1].toFixed(1)} L${P(1,1,K)[0].toFixed(1)},${P(1,1,K)[1].toFixed(1)} L${P(0,1,K)[0].toFixed(1)},${P(0,1,K)[1].toFixed(1)} L${P(0,1,0)[0].toFixed(1)},${P(0,1,0)[1].toFixed(1)}"/>`
    +`</g>`;
  // X 刻度（前底边）
  ...
  // Y 刻度（前左边）
  const yTicks=yTicksNice,yStep=yTicks.length>1?(yTicks[1]-yTicks[0]):1;
  out+=`<g data-object="axis-y" class="chart-object" stroke="${ac}" stroke-width="${aw}" fill="none">`;
  if(s.showYTicks)yTicks.forEach(v=>{const ny=yN(v);if(ny<0||ny>1)return;const p=P(0,ny,0);out+=wfLine(p,[p[0]-s.tickLength,p[1]],ac,aw);});
  out+='</g>';
  yTicks.forEach(v=>{const ny=yN(v);if(ny<0||ny>1)return;const p=P(0,ny,0);out+=`<text data-object="axis-y" class="chart-object" x="${(p[0]-s.tickLength-6).toFixed(1)}" y="${(p[1]+4).toFixed(1)}" text-anchor="end" font-size="${s.yTickSize}" fill="${s.yTickColor}">${formatAxisNumber(v,s.yTickDecimals,yStep,false)}</text>`;});
  // Z 轴（右斜边）层标签
  if(K>=1&&s.wfZLabels){
    for(let k=0;k<=K;k++){const p=P(1,0,k),lx=p[0]+11+((k%2)*18),ly=p[1]+4;
      out+=wfLine(p,[p[0]+7,p[1]],ac,aw);
      out+=`<text data-object="axis-z" class="chart-object" x="${lx.toFixed(1)}" y="${ly}" font-size="${s.wfZLabelSize}" fill="${ac}">${esc(gs[k])}</text>`;}
    if(s.wfZAxisTitle){
      const mid=P(1,0,K/2),ang=Math.atan2(-dy,dx)*180/Math.PI,L=Math.hypot(dx,dy)||1,nxO=dy/L,nyO=dx/L;
      out+=`<text data-object="axis-z" class="chart-object" x="${(mid[0]+nxO*30).toFixed(1)}" y="${(mid[1]+nyO*30-6).toFixed(1)}" font-size="${s.zTitleSize||s.yTickSize}" font-style="italic" fill="${ac}" transform="rotate(${ang.toFixed(1)} ${(mid[0]+nxO*30).toFixed(1)} ${(mid[1]+nyO*30-6).toFixed(1)})">${esc(s.wfZAxisTitle)}</text>`;
    }
  }
  // 轴标题
  if(s.xTitleVisible&&s.xTitle){const p=P(0.5,0,0);out+=`<text data-object="axis-x" data-drag="xTitle" class="chart-object draggable" x="${p[0].toFixed(1)}" y="${(p[1]+64).toFixed(1)}" text-anchor="middle" font-size="${s.xTitleSize}" font-weight="${s.xTitleWeight||s.globalFontWeight||400}" fill="${s.xTitleColor}">${esc(convertedXTitle())}</text>`;}
  if(s.yTitleVisible&&s.yTitle){const p=P(0,0.5,0);out+=`<text data-object="axis-y" data-drag="yTitle" class="chart-object draggable" transform="translate(${p[0]-64} ${p[1].toFixed(1)}) rotate(-90)" text-anchor="middle" font-size="${s.yTitleSize}" font-weight="${s.yTitleWeight||s.globalFontWeight||400}" fill="${s.yTitleColor}">${esc(s.yTitle)}</text>`;}
    if(s.wfLegend&&K>=1){const lx0=P(1,1,K)[0]+14,ly0=P(1,1,K)[1]+8;out+=`<g data-object="legend" class="chart-object" font-size="${s.wfLegendSize||11}">`;for(let k=0;k<=K;k++){const cc=colors[k%colors.length];out+=`<rect x="${lx0.toFixed(1)}" y="${(ly0+k*17).toFixed(1)}" width="11" height="11" fill="${cc}" stroke="${darken(cc,.2)}" stroke-width=".6"/>`;out+=`<text x="${(lx0+15).toFixed(1)}" y="${(ly0+k*17+10).toFixed(1)}" fill="#333">${esc(gs[k])}</text>`;}out+=`</g>`;}
  return out;
}
