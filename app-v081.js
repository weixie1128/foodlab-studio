...
const defaultChartSettings = {
  ...
  wfLayerDx:44, wfLayerDy:56, wfFill:true, wfFillOpacity:.55, wfZLabels:true, wfLineEndLabels:false, wfZLabelSize:11, wfZAxisTitle:'Time', zTitleSize:12,
  background:'#ffffff'
};
...
function init(){
  ...
  renderDataPreview();
  showView('plan');
  if('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v='+(window.FOODLAB_BUILD||'0.81.0')).catch(()=>{});
}
...
function parseStructuredExperimentMatrix(matrix){
  ...
  // v0.81.0 单层宽表守卫：第二行若是实验数值（而非每组连续的平行编号/标签），
...
/* ===== v0.81.0 光谱瀑布图（真 3D 坐标盒，Matlab/Origin 风格）=====
...
function renderWaterfall(W,H,M,plotW,plotH,xvals,gs,colors,b){
  ...
  out+=`<g data-object="axis-box" class="chart-object" stroke="${ac}" stroke-width="${aw}" fill="none" stroke-linecap="square">`
    +`<path d="M${P(0,0,0)[0]},${P(0,0,0)[1]} L${P(1,0,0)[0]},${P(1,0,0)[1]} L${P(1,0,K)[0].toFixed(1)},${P(1,0,K)[1].toFixed(1)} L${P(1,1,K)[0].toFixed(1)},${P(1,1,K)[1].toFixed(1)} L${P(0,1,K)[0].toFixed(1)},${P(1,1,K)[1].toFixed(1)} L${P(0,1,0)[0]},${P(0,1,0)[1]} L${P(0,0,0)[0]},${P(0,0,0)[1]}"/>`
    +`</g>`;
  // X 刻度（前底边）
  ...
  if(K>=1&&s.wfZLabels){
    for(let k=0;k<=K;k++){const p=P(1,0,k),lx=p[0]+11+((k%2)*18),ly=p[1]+4;
      ...
      out+=`<text data-object="axis-z" x="${lx.toFixed(1)}" y="${ly}" font-size="${s.wfZLabelSize}" fill="${ac}">${esc(gs[k])}</text>`;}
    if(s.wfZAxisTitle){
