...
function ensureComposeGridCapacity(){
  const b=state.figureBoard;b.columns=clamp(Number(b.columns)||2,1,6);b.rows=clamp(Number(b.rows)||1,1,8);const totalUnits=(b.items||[]).reduce((s,it)=>s+Math.max(1,Number(it.colspan)||1),0);b.rows=Math.max(b.rows,Math.ceil(totalUnits/(b.columns||1))||1);
}
...
function composeSvgMarkup(interactive=true){
  ...
  let body=`<rect width="${W}" height="${H}" fill="${b.background}"/>`,curCol=0,flowRow=0;
  items.forEach((item,i)=>{const span=Math.max(1,Math.min(Number(item.colspan)||1,cols));if(curCol>0&&curCol+span>cols){curCol=0;flowRow++}const cc=curCol,r=flowRow,x=pad+cc*(cellW+gap),y=pad+r*(cellH+gap),w=cellW*span+gap*(span-1);curCol+=span;if(item.kind==='image'){body+=`<image x="${x}" y="${y}" width="${w}" height="${cellH}" href="${escAttr(item.src)}" preserveAspectRatio="xMidYMid meet"/>`}else{const srcSvg=b.trimWhitespace!==false?trimSvgWhitespace(item.svg,8):item.svg,prefix=`p${i}_`,prefixed=prefixSvgIds(srcSvg,prefix),inner=svgInner(prefixed),pres=svgRootPresentation(srcSvg),style=pres.style?` style="${escAttr(pres.style)}"`:'',ff=pres.fontFamily?` font-family="${escAttr(pres.fontFamily)}"`:'',fw=pres.fontWeight?` font-weight="${escAttr(pres.fontWeight)}"`:'';body+=`<svg x="${x}" y="${y}" width="${w}" height="${cellH}" viewBox="0 0 ${item.width} ${item.height}" preserveAspectRatio="xMidYMid meet"${style}${ff}${fw}>${inner}</svg>`}if(b.panelBorder)body+=`<rect x="${x}" y="${y}" width="${w}" height="${cellH}" fill="none" stroke="${b.panelBorderColor}" stroke-width="${b.panelBorderWidth}"/>`;if(b.labelEnabled){let lx=x+b.labelInsetX,ly=y+b.labelSize+b.labelInsetY,anchor='start';if(b.labelPosition==='top-right'){lx=x+w-b.labelInsetX;anchor='end'}else if(b.labelPosition==='outside-top-left'){lx=x-b.labelInsetX;ly=y-b.labelInsetY}body+=`<text x="${lx}" y="${ly}" text-anchor="${anchor}" font-family="${escAttr(b.labelFont||'Arial')},sans-serif" font-size="${b.labelSize}" font-weight="${b.labelWeight}" fill="${b.labelColor}">${esc(item.label||indexLetter(i).toUpperCase())}</text>`}});
