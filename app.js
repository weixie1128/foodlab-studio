...
function composeSvgMarkup(interactive=true){
  ...
  pos.forEach(p=>{
    ...
    if(item.kind==='image'){body+=`<image x="${x}" y="${y}" width="${w}" height="${ph}" href="${escAttr(item.src)}" preserveAspectRatio="xMidYMid meet"/>`}
    ...
    if(interactive)body+=`<rect x="${x}" y="${y}" width="${w}" height="${ph}" fill="transparent" data-compose-panel="${p.i}" style="cursor:pointer"/>`;
  });
