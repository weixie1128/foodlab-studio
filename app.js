...
function bindComposeAnnotationInteractions(){
  ...
  $$('[data-compose-panel]').forEach(el=>el.addEventListener('click',e=>{e.stopPropagation();state.figureBoard.selected=Number(el.dataset.composePanel);renderComposeItemList();renderComposeSettings();renderComposePreviewOnly()}));
