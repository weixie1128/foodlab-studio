...
const defaultChartSettings = {
  ...
  wfLayerDx:34, wfLayerDy:46, wfFill:true, wfFillOpacity:.92, wfZLabels:true, wfLineEndLabels:false, wfZLabelSize:11, wfZAxisTitle:'Time', zTitleSize:12,
  background:'#ffffff'
};
...
function init(){
  ...
  renderDataPreview();
  showView('plan');
  if('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v='+(window.FOODLAB_BUILD||'0.80.0')).catch(()=>{});
}
...
function parseStructuredExperimentMatrix(matrix){
  ...
  // v0.80.0 单层宽表守卫：第二行若是实验数值（而非每组连续的平行编号/标签），
...
/* ===== v0.80.0 光谱瀑布图（真 3D 坐标盒，Matlab/Origin 风格）=====
...
function wfLayout(n,W,H){
  ...
  dx=Math.min(dx,Math.max(0,(W-l-r0-280))/Math.max(1,K));
  dy=Math.min(dy,Math.max(0,(H-t0-b-250))/Math.max(1,K));
...
function renderWaterfall(W,H,M,plotW,plotH,xvals,gs,colors,b){
  if(b.min<0&&state.chartData.every(d=>d.mean>=0))b=Object.assign({},b,{min:0});
  ...
  let out='';
  // 1) 后墙（z=K，淡黄）
  out+=wfPoly([P(0,0,K),P(1,0,K),P(1,1,K),P(0,1,K)],'fill="#fbf5d2" stroke="none"');
  xs.ticks.forEach(t=>out+=wfLine(P(t.n,0,K),P(t.n,1,K),'#e6dca8',1));
  chartYTicks(b).forEach(v=>{const ny=yN(v);if(ny>=0&&ny<=1)out+=wfLine(P(0,ny,K),P(1,ny,K),'#e6dca8',1);});
  // 2) 左墙（x=0，淡青）
  out+=wfPoly([P(0,0,0),P(0,0,K),P(0,1,K),P(0,1,0)],'fill="#dcefee" stroke="none"');
  chartYTicks(b).forEach(v=>{const ny=yN(v);if(ny>=0&&ny<=1)out+=wfLine(P(0,ny,0),P(0,ny,K),'#b6d6d6',1);});
  for(let k=0;k<=K;k++)out+=wfLine(P(0,0,k),P(0,1,k),'#b6d6d6',1);
  // 3) 底面（y=0，灰白）
  out+=wfPoly([P(0,0,0),P(1,0,0),P(1,0,K),P(0,0,K)],'fill="#f1f1ee" stroke="none"');
  xs.ticks.forEach(t=>out+=wfLine(P(t.n,0,0),P(t.n,0,K),'#d3d5d0',1));
  for(let k=0;k<=K;k++)out+=wfLine(P(0,0,k),P(1,0,k),'#d3d5d0',1);
  // 4) 各层片（后→前，实色遮挡）
  for(let k=K;k>=0;k--){
  ...
  out+=`<g data-object="axis-box" class="chart-object" stroke="${ac}" stroke-width="${aw}" fill="none" stroke-linecap="square">`
    +`<path d="M${P(0,0,0)[0]},${P(0,0,0)[1]} L${P(1,0,0)[0]},${P(1,0,0)[1]} L${P(1,0,K)[0].toFixed(1)},${P(1,0,K)[1].toFixed(1)} L${P(1,1,K)[0].toFixed(1)},${P(1,1,K)[1].toFixed(1)} L${P(0,1,K)[0].toFixed(1)},${P(1,1,K)[1].toFixed(1)} L${P(0,1,0)[0]},${P(0,1,0)[1]} L${P(0,0,0)[0]},${P(0,0,0)[1]} M${P(0,0,K)[0].toFixed(1)},${P(0,0,K)[1].toFixed(1)} L${P(0,1,K)[0].toFixed(1)},${P(0,1,K)[1].toFixed(1)}"/>`
    +`</g>`;
  // X 刻度（前底边）
  ...
  // Z 轴（右斜边）层标签
  if(K>=1&&s.wfZLabels){
    for(let k=0;k<=K;k++){const p=P(1,0,k);
      out+=wfLine(p,[p[0]+7,p[1]],ac,aw);
      out+=`<text data-object="axis-z" x="${(p[0]+11).toFixed(1)}" y="${(p[1]+4).toFixed(1)}" font-size="${s.wfZLabelSize}" fill="${ac}">${esc(gs[k])}</text>`;}
    if(s.wfZAxisTitle){
      const mid=P(1,0,K/2),ang=Math.atan2(-dy,dx)*180/Math.PI,L=Math.hypot(dx,dy)||1,nxO=dy/L,nyO=dx/L;
      out+=`<text data-object="axis-z" x="${(mid[0]+nxO*30).toFixed(1)}" y="${(mid[1]+nyO*30-6).toFixed(1)}" font-size="${s.zTitleSize||s.yTickSize}" font-style="italic" fill="${ac}" transform="rotate(${ang.toFixed(1)} ${(mid[0]+nxO*30).toFixed(1)} ${(mid[1]+nyO*30-6).toFixed(1)})">${esc(s.wfZAxisTitle)}</text>`;
    }
  }
  // 轴标题
